...
class GarminHAWidgetDelegate extends WatchUi.BehaviorDelegate {
    ...
    function initialize() {
        ...
        _view = WatchUi.getActiveView() as GarminHAWidgetView;
    }
    ...
    function onKey(keyEvent as WatchUi.KeyEvent) as Lang.Boolean {
        ...
        if (_view._keySequenceHandler != null) {
            var currentSequence = _view._keySequenceHandler.handleKeyPress(keyString);
    ...
    function onMenu() as Lang.Boolean {
        WatchUi.pushView(new Rez.Menus.MainMenu(), new GarminHAWidgetMenuDelegate(), WatchUi.SLIDE_UP);
        return true;
    }
}
...
class GarminHAWidgetMenuDelegate extends WatchUi.MenuInputDelegate {
    ...
    function onMenuItem(item as WatchUi.Symbol) as Void {
        var view = WatchUi.getActiveView();
        
        if (item == :refresh_config) {
            if (view instanceof GarminHAWidgetView) {
                (view as GarminHAWidgetView).refreshConfig();
            }
        } else if (item == :clear_cache) {
            if (view instanceof GarminHAWidgetView) {
                (view as GarminHAWidgetView).clearCache();
            }
        } else if (item == :settings) {
            // Settings are handled by Connect IQ app
        }
    }
}