...
class ConfigManager {
    ...
    function loadSettings() as Void {
        ...
        // If HA URL is not set, try to derive from config URL
        if (_haUrl == null || _haUrl.equals("https://your-ha-instance.com")) {
    ...
    function deriveHaUrlFromConfig() as Void {
        if (_configUrl != null && !_configUrl.equals("https://example.com/ha-config.json")) {
            ...
            if (protocolEnd != null) {
                ...
                var pathStart = _configUrl.find("/", domainStart);
                if (pathStart != null) {
    ...
    function loadConfig(callback as Lang.Method) as Void {
        loadConfig(callback, false);
    }

    function refreshConfig(callback as Lang.Method) as Void {
        loadConfig(callback, true);
    }

    function loadConfig(callback as Lang.Method, forceRefresh as Lang.Boolean) as Void {
        _callback = callback;
